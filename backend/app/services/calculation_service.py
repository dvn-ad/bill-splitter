...
def recalculate_split_by_item(invoice: Invoice, assignments: Dict[str, List[str]]) -> Dict[str, float]:
    """
    Recalculates the total for each person based on assigned items,
    applying tax and service charge proportionally.
    """
    person_subtotals = {name: 0.0 for name in assignments.keys()}
    
    # Calculate how many people are assigned to each item (sharing)
    item_usage = {}
    ...
    # Calculate final totals with proportional tax and service charge
    results = {}
